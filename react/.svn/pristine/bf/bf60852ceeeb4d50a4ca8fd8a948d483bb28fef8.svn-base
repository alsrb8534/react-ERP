...
import { closeSalaryWithSlipRequest } from 'ERP/HR/salary/reducer/SalaryReducer'; //임금 2020-11-19 박미노
import { deliveryDivisionStart } from 'ERP/LOGISTIC/Sales/action/SalesAction'; //납품 2020-11-21 박미노
...
const mapStateToProps = state => {
    console.log('분개 리턴 컨테이너' + JSON.stringify(state.AccReducer.journalList));
    return {
        slipFormList: state.AccReducer.slipFormList,
        journalList: state.AccReducer.journalList,
        error: state.AccReducer.error,
        journalDetailList: state.AccReducer.journalDetailList
    };
};
