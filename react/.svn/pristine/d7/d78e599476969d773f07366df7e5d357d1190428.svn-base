...
import * as types from 'ERP/LOGISTIC/LogisticsInfo/action/LogisticsInfoActionType';
...
function* codeInfo(action) {
    try {
        const { data } = yield axios.get('http://localhost:8282/logi/base/searchCodeList.do', null);
        if (data.errorCode > 0) {
...
export function* detailCodeInfoSaga() {
...
export function* warehouseInfoSaga() {
...
export default function* LogisticsInfoSaga() {
    yield all([call(codeInfoSaga), call(detailCodeInfoSaga), call(warehouseInfoSaga)]);
}
