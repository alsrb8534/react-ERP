...
function WorkInstruction(props) {
    ...
    const column = {
        columnDefs: [
            ...
            { headerName: '품목분류', field: 'orderOrProductionStatus' },
        ...
        <>
            <div>
                ...
                >
                    ...
                    <Button variant="contained" color="secondary"  onClick={onClicked3}>
                    세션스토리지 확인하기
                    </Button>
                </MyGrid>
                <MyDialog open={searchOpenDialog} close={close} maxWidth={'200px'}>
                    <ForwardOrderDialog data={data} close={close} setList={setList}
                    gridApi={gridApi}
                    />
                </MyDialog>
            </div>
