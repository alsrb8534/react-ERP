...
function WorkOrderDialog(props) {
    ...
    const contractSearch = useCallback(params => {
        ...
        if (result) {
            Axios.get('http://localhost:8282/logi/production/workOrder.do', {
                params: {
    ...
    useEffect(() => {
        Axios.get('http://localhost:8282/logi/production/showWorkOrderDialog.do', {
            params: {
                mrpGatheringNo: props.data.mrpGatheringNo
            }
        })
        ...
        //사업장
        Axios.get('http://localhost:8282/hr/basicInfo/searchWorkplace.do', {
            params: {
                companyCode: 'COM-01'
            }
        })
