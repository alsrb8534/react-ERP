...
export const setSearchStockListAxios = (setSearchStockList) => {
    ...
    axios.get("http://localhost:8282/logi/purchase/searchStockList.do").then(({data}) => {
        if(data.errorCode < 0){
            return Swal.fire({
                icon: "error",
                title: data.errorMsg
...
export const setSearchOrderInfoListOnDelivery = (setOrderInfoList) => {
    axios.get("http://localhost:8282/logi/purchase/searchOrderInfoListOnDelivery.do").then(({data}) => {
        if(data.errorCode < 0){
            return Swal.fire({
                icon: "error",
                title: data.errorMsg
...
export const setWarehousing = (orderNoList) => {
    axios.get("http://localhost:8282/logi/purchase/warehousing.do",{
        params:{
